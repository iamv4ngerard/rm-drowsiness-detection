...
personal_avg_ear = 0.25   # Baseline awal
current_label = 0         # 0 = Normal, 1 = Ngantuk (Untuk pengumpulan data training)
...
def check_head_yaw(landmarks):
    ...
    return get_head_yaw_diff(landmarks) > 0.04
...
def check_head_drop(landmarks):
    rasio = get_head_pitch_ratio(landmarks)
...
with open(csv_filename, mode='w', newline='') as file:
    ...
    writer.writerow([
        'Waktu', 
        'EAR_Aktual', 
        'Norm_EAR',        # Novelty: EAR yang dinormalisasi dengan bentuk wajah orang tersebut
        'MAR_Aktual', 
        'PERCLOS', 
        'Head_Pitch_Ratio',# Angka untuk model ML menunduk
        'Head_Yaw_Diff',   # Angka untuk model ML menoleh
        'Intensitas_Cahaya', 
        'Adaptive_CLAHE_Aktif', 
        'Label'            # GROUND TRUTH: 0=Sadar, 1=Ngantuk (Wajib ada untuk training ML)
    ])
...
with vision.FaceLandmarker.create_from_options(options) as landmarker:
    ...
    while cap.isOpened():
        ...
        if result.face_landmarks:
            for landmarks in result.face_landmarks:
                ...
                if is_waiting_to_start:
                    ...
                    # Ekstraksi Fitur Tambahan (Continuous)
                    ...
                    with open(csv_filename, mode='a', newline='') as file:
                        ...
                        writer.writerow([
                            current_time,
                            round(ear_value, 4),           # EAR
                            round(norm_ear, 4),            # EAR Normalized
                            round(mar_value, 4),           # MAR
                            round(current_perclos, 4),     # PERCLOS
                            round(head_pitch, 4),          # Pitch
                            round(head_yaw, 4),            # Yaw
                            round(avg_brightness, 2),      # Light Intensity
                            1 if is_low_light else 0,      # CLAHE status
                            current_label                  # Label Input dari User (1/0)
                        ])
        ...
        # Petunjuk mode perekaman (Labeling)
        label_text = "NGANTUK (1)" if current_label == 1 else "SADAR (0)"
        label_color = (0, 0, 255) if current_label == 1 else (0, 255, 0)
        cv2.putText(frame, f"MODE REKAM: {label_text} [Tekan 0/1 untuk ubah]", (30, frame.shape[0] - 60), 
        ...
        # Ganti label pencatatan (0 = Sadar, 1 = Ngantuk)
        elif key == ord('0'):
            current_label = 0
            print("Mode Perekaman: SADAR (0)")
        elif key == ord('1'):
            current_label = 1
            print("Mode Perekaman: NGANTUK (1)")
